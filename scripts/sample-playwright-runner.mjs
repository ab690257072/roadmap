...
function loadSamples(slug) {
  return JSON.parse(readFileSync(join(ROOT, `.cursor/planning/diagram-checks/${slug}-samples.json`), 'utf8'));
}
  ...
  for (const { slug, title } of roadmaps) {
    ...
    for (const sample of samples) {
      ...
      try {
        ...
        await page.locator('.ant-drawer').screenshot({ path: out });
        results.push({ key, label: sample.label, ok: textCount > 0, textCount, out });
      } catch (err) {
        results.push({ key, label: sample.label, ok: false, error: String(err.message || err) });
      }
    }
    const report = { slug, total: samples.length, pass: results.filter((r) => r.ok).length, results };
    writeFileSync(join(dir, 'report.json'), JSON.stringify(report, null, 2));
    summary.push({ slug, total: report.total, pass: report.pass, fail: report.total - report.pass, failures: results.filter((r) => !r.ok) });
  }
  return summary;
}
