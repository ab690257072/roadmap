#!/usr/bin/env node
/**
 * Generates src/roadmaps/sql/teaching-handcrafted.ts
 * 112 nodes — unique Chinese copy + semantic Excalidraw per node.
 */
...
function loadNodes() {
  const data = readFileSync('src/roadmaps/sql/data.ts', 'utf8');
...
function loadLessonSpecs() {
  const src = readFileSync('src/roadmaps/sql/teaching.ts', 'utf8');
  const start = src.indexOf('export const SQL_TEACHING_LESSONS');
  const end = src.indexOf('export function hasSqlTeachingLesson');
  let body = src.slice(start, end);
  body = body.replace('export const SQL_TEACHING_LESSONS: Record<string, TeachingLesson> = ', '');
  body = body.replace(/sqlLesson\(/g, '(');
  body = body.replace(/ as SqlDiagramKind/g, '');
...
const STAGE_HINT = {
  1: '关系模型基础', 2: 'SQL 基础语法', 3: 'DDL 与表结构', 4: 'DML 数据变更', 5: 'SELECT 查询',
  6: '聚合与分组', 7: '约束与键', 8: '子查询', 9: 'JOIN 连接', 10: '标量与函数',
  11: '字符串函数', 12: 'JOIN 进阶', 13: '视图', 14: '索引与优化', 15: '日期时间',
  16: '事务与 ACID', 17: '安全与权限', 18: '窗口函数', 19: '高级 SQL',
};
...
function craftRoute(spec, zh, stage) {
  ...
  const hint = STAGE_HINT[stage] || 'SQL 查询';
  return [
...
function craftContent(spec, node, index) {
  ...
  const scene = `你在写或排查 SQL 时遇到「${zh}」。先确认它面对的是「${spec.subject}」，再判断语句会读、改、约束还是优化什么。`;
  ...
  const experiment = spec.experiment.startsWith('用一张') ? spec.experiment
    : `建临时表插入样例数据，练 \`${spec.command}\`：先 SELECT 再改；涉及 DELETE/UPDATE/DROP 必须先确认目标行。`;
...
if (allSpecs.length !== 112) {
  console.error('Expected 112 nodes, got', allSpecs.length);
  process.exit(1);
}
...
${routeStr}
    ...
    rawDiagram: SQL_DIAGRAMS['${s.slug}'],
  },`;
}).join('\n');
...
writeFileSync('src/roadmaps/sql/teaching-handcrafted.ts', out);
