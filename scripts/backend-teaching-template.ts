  ...
  switch (spec.diagram) {
    case 'journey':
      return excalidrawScene([
        excalidrawText(spec.id + '-title', 76, 18, title + '：前端转后端学习路径', 25, 620),
    ...
    default:
      return excalidrawScene([
        excalidrawText(spec.id + '-title', 76, 18, title + '：后端一环', 25, 620),
...
function backendLesson(spec: BackendSpec): TeachingLesson {
  return {
    scene: spec.scene,
    core: spec.core,
    route: spec.route,
    ...
    rawDiagram: backendDiagram(spec),
  };
}
